    ...
    <section className="py-24 px-6 border-b border-gray-300 relative overflow-hidden bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { emoji: '🅰️', title: 'Le Timbre', desc: 'Revenu passif : Chaque mail externe = 0,10€', color: '#0066FF' },
            { emoji: '🅱️', title: 'L\'Abonnement', desc: 'Cœur du business : KLAR PRO à 5€/mois', color: '#00CC88' },
            { emoji: '🅾️', title: 'Vanity Names', desc: 'Adresses courtes : tom@klar.app à 50€ à vie', color: '#9900FF' }
          ].map((item, index) => (
            <motion.div 
              key={index}
              className="border-2 border-gray-300 bg-white p-6 rounded-2xl cursor-pointer group"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: index * 0.1 }}
            ...
            >
              <motion.div 
                className="text-[40px] leading-none tracking-tight mb-2"
                whileHover={{ rotate: [0, -10, 10, -10, 0] }}
                transition={{ duration: 0.5 }}
              >
                {item.emoji}
              </motion.div>
              <h4 className="text-[24px] leading-tight mb-2 text-black">{item.title}</h4>
